...
%as a game where the players are the agents argumenting, and the set of actions
%available to them is the set of argumentative moves (eg. asserting or challenging
...
%\begin{enumerate}
    %\item \emph{procedural rules} -- about the order and type of arguments each
        %debatee is allowed to state \begin{itemize}
        %\item \emph{one-speaker debate} -- two arguments are revealed at once by
            %one debatee
        %\item \emph{simultaneous debate} -- the two debatees simultanously each
            %reveal one argument
        %\item \emph{sequential debate} -- debatees take turns, each revealing one
            %argument at once
    %\end{itemize}
%\end{enumerate}
...
In this subsection core concepts of game theory are introduced in order to
support of the following sections.
...
\begin{Def}[Nash equilibrium \cite{carte}]
...
% TODO paralela cu ce s-a facut la curs saptamana 16 Jan.
